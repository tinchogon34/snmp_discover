...
gateway, mask, community, options = *ARGV

case options
when '-h' then 
  puts "Sintaxis => discover.rb [gateway] [mask] [community] [options]"
  exit
end
...
SNMP::Manager.open(Host: gateway, Community: (community || 'public')) do |manager|
